...
def split(train_pct:float, valid_pct:float, inp_dir, out_dir, remove_suffix, suffix, seed=None):
  ...
  for name, idx in zip(names, (idxs[:cut1], idxs[cut1:cut2+cut1], idxs[cut2+cut1:])):
    with open(path.join(out_dir, name), 'w') as file:
      file.writelines([items[i].replace(suffix, '') +'\n' for i in idx])
...
if __name__ == '__main__':
  ...
  parser.add_argument('--valid_pct', type=float, help='how much the valid data should be. default:0', default=0.)
  parser.add_argument('--test_pct', type=float, help='how much the test data should be. default: 0', default=0.)
  parser.add_argument('--seed', type=int, help='seed would be set to random stat.e')
  parser.add_argument('--remove_suffix', type=bool, help='remove suffix, defalt: True', default=True)
  parser.add_argument('--suffix', type=str, help='file suffix, defalt: .xml', default='.xml')
